...
import { 
  PlusCircle, Edit, Trash2, ChevronDown, ChevronUp, 
  Coffee, ShoppingBag, Clock, DollarSign, Users, Search,
  AlertTriangle, CheckCircle, XCircle, RefreshCw, Zap,
  TrendingUp, BarChart2, PieChart, Package, Truck, CheckSquare
} from 'lucide-react';
...
export default function EnhancedAdminPanel() {
  ...
  const fetchAllMenuItems = async () => {
    ...
    try {
      const menuItemsPromises = restaurants.map(restaurant => 
        axios.get(`http://localhost:5000/api/restaurants/${restaurant._id}/menu`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      );
      const menuItemsResponses = await Promise.all(menuItemsPromises);
      const allMenuItems = menuItemsResponses.flatMap(response => response.data);
  ...
  const addMenuItem = async (e) => {
    ...
    try {
      ...
      setNewItem({ name: '', description: '', price: '', image: '', restaurantId: '' });
  ...
  const addRestaurant = async (e) => {
    ...
    try {
      ...
      setNewRestaurant({ name: '', cuisine: '', image: '', isLateNight: false, location: '' });
  ...
  const updateOrderStatus = async (orderId, status) => {
    ...
    try {
      ...
      setOrders(orders.map(order => 
        order._id === orderId 
          ? { ...order, ...response.data, user: order.user } 
          : order
      ));
  ...
  const deleteRestaurant = async (restaurantId) => {
    if (window.confirm('Are you sure you want to delete this restaurant? This will also delete all associated menu items.')) {
      ...
      try {
        ...
        setMenuItems(menuItems.filter(item => item.restaurant !== restaurantId));
        setSuccess('Restaurant and associated menu items deleted successfully!');
      } catch (error) {
  ...
  const filteredMenuItems = selectedRestaurant
    ? menuItems.filter(item => item.restaurant === selectedRestaurant)
    : menuItems;
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-gradient-to-br from-orange-100 to-yellow-100 min-h-screen">
      <motion.h1 
      ...
      <div className="flex justify-center mb-8 space-x-4">
        {['dashboard', 'restaurants', 'menu', 'orders'].map((tab) => (
          <motion.button 
            key={tab}
            className={`px-4 py-2 rounded-lg ${activeTab === tab ? 'bg-orange-500 text-white' : 'bg-white text-orange-500 border border-orange-500'}`}
      ...
      <div className="mb-4">
        <div className="relative">
          ...
          />
          <Search className="absolute left-3 top-2.5 text-orange-400" size={20} />
        </div>
      </div>
        ...
        {activeTab === 'dashboard' && (
          ...
          >
            ...
            <div className="col-span-full">
              <h3 className="text-xl font-semibold mb-4 text-orange-700">Sales Chart</h3>
              <div className="bg-white p-4 rounded-lg shadow-lg">
                <Bar
                  data={{
                    labels: analytics.monthlySales.map(([month]) => month),
                    datasets: [
                      {
                        label: 'Monthly Sales',
                        data: analytics.monthlySales.map(([, sales]) => sales),
            ...
            <div className="col-span-full md:col-span-1">
              <h3 className="text-xl font-semibold mb-4 text-orange-700">Top Selling Items</h3>
              <div className="bg-white p-4 rounded-lg shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredRestaurants.map((restaurant) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <h3 className="font-bold text-xl mb-2 text-orange-700">{restaurant.name}</h3>
                    <p className="text-orange-600 mb-2">{restaurant.cuisine}</p>
                    <p className="text-sm text-orange-500 mb-4">
                      {restaurant.isLateNight ? 'Late Night Available' : 'Not Available Late Night'}
                    </p>
                    <p className="text-sm text-orange-500 mb-4">
                    ...
                    <div className="flex justify-between">
                      <motion.button
                        ...
                        className="bg-orange-500 text-white px-3 py-1 rounded hover:bg-orange-600 transition duration-300 flex items-center"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredMenuItems.map((item) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <h3 className="font-bold text-xl mb-2 text-orange-700">{item.name}</h3>
                    <p className="text-orange-600 mb-2">{item.description}</p>
                    <p className="text-lg font-semibold mb-4 text-orange-500">Price: ₹{parseFloat(item.price).toFixed(2)}</p>
                    <p className="text-sm text-orange-500 mb-4">
                      Restaurant: {restaurants.find(r => r._id === item.restaurant)?.name || 'Unknown'}
                    </p>
                    <div className="flex justify-between">
                      <motion.button
                      ...
                      <motion.button
                        onClick={() => deleteMenuItem(item._id, item.restaurant)}
                        className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition duration-300 flex items-center"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
              ...
              {filteredOrders.map((order) => (
                ...
                >
                  <div className="p-6">
                    ...
                    {/* Wrap the expandable section with AnimatePresence */}
                    <AnimatePresence>
      ...
      {editingItem && (
        ...
        >
          <motion.div 
            ...
            transition={{ duration: 0.3 }}
          >
            <h3 className="text-lg font-bold mb-4 text-orange-700">Edit Menu Item</h3>
      ...
      {editingRestaurant && (
        ...
        >
          <motion.div 
            ...
            transition={{ duration: 0.3 }}
          >
            <h3 className="text-lg font-bold mb-4 text-orange-700">Edit Restaurant</h3>
            <form onSubmit={editRestaurantHandler} className="space-y-4">
              ...
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) => setEditingRestaurant({...editingRestaurant, isLateNight: e.target.checked})}
                  className="mr-2"
                />
                <label htmlFor="editIsLateNight" className="text-orange-700">Available for Late Night Delivery</label>
              </div>
              <input
                type="text"
